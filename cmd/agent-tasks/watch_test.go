...
//  2. Starts "agent-tasks watch" in the background with a short timeout.
//  3. Creates a task via "agent-tasks add" while watch is running.
...
func TestCLI_Watch_Smoke(t *testing.T) {
	...
	_, stderr, code := runCmd(t, binPath, dir, "add", "watch-smoke-task")
	if code != 0 {
